...
Completed 2026-08-10 with `scripts/certify-release-supply-chain.sh` (evidence in `docs/certification/release-supply-chain-20260810-125405-32130/`, `SUMMARY.json` `failures: 0`, secret scan 0 findings):
...
- **published SBOM assets**: CycloneDX 1.5 SBOM create/verify/schema against the pristine HEAD worktree (1211 components) with a mutation negative test; real SBOM bound to HEAD. Publishing SBOM assets on a release remains **blocked** (release creation forbidden);
