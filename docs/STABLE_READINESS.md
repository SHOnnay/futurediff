...
- every supported provider surface certified: GitHub branch publish (`builtin.github.branch-publish`), GitHub draft pull request (`builtin.github.draft-pull-request`), and Slack message outbox (`builtin.slack.message-outbox`);
...
- Slack real-mutation evidence remains blocked on dedicated test credentials: `FUTUREDIFF_SLACK_TOKEN` plus a dedicated channel (`--slack`); deterministic Slack coverage (post, status, metadata recovery, exactly-once reconciliation, token redaction) is fully recorded;
...
- concrete provider-integration evidence for every supported provider surface — **completed 2026-08-09** (see the provider-integration certification above).
